    ...
    if (attributes) {
      for (i in attributes) {
        if (attributes[i] instanceof Object) {
          for (j in attributes[i]) node[i][j] = attributes[i][j];
        } else node[i] = attributes[i];
      }
    }

    if (events) {
      for (i in events) node.addEventListener(i, events[i]);
    }
...
HTMLElement.prototype.append = function (node) {
  if (typeof node == "string") node = elem.apply(null, arguments);
