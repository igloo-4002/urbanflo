on("ready", function () {
  ...
  Settings.prototype = {
    init: function (display, internal, fringeFactor, count, enabled) {
      ...
      label = elem("<label>", {
        textContent: "Count",
        title: "Count per hour per kilometer",
      });
      this.count = elem("<input>", {
        type: "number",
        min: 0.2,
        max: 100,
        step: 0.1,
  ...
  RoadTypes.prototype = {
    init: function (category, typeList) {
      ...
      var types = elem("<div>", {
        className: "roadTypes " + category.toLowerCase(),
      });
  ...
  categories["Pedestrians"] = [
    ...
    "steps",
    "stairs",
  ]; //"Pedestrians" has also the "highway" key in OSM, this will be transformed in startBuild()
  categories["Railway"] = [
    "preserved",
    "tram",
    "subway",
  ...
  var canvasRect = [0.1, 0.1, 0.75, 0.9];
  ...
  function changeMousePointer(x, y, down) {
    ...
    if (lequal(x0 - t, x, x1 + t) && lequal(y0 - t, y, y1 + t)) {
      if (lequal(y0 - t, y, y0 + t)) cursor += "n";
      else if (lequal(y1 - t, y, y1 + t)) cursor += "s";

      if (lequal(x0 - t, x, x0 + t)) cursor += "w";
      else if (lequal(x1 - t, x, x1 + t)) cursor += "e";
  ...
  canvas.on("mousemove", function (evt) {
    changeMousePointer(evt.clientX, evt.clientY);
    var dx = (evt.clientX - mouse.x) / canvas.width,
      dy = (evt.clientY - mouse.y) / canvas.height;
    mouse.x = evt.clientX;
    mouse.y = evt.clientY;
    if (mouse.area !== null) {
      if (mouse.area[1] == "n") {
  ...
  var checkOrUncheckAll = function () {
    Array.from(
      document.querySelectorAll(
        ".roadTypes." + this.getAttribute("id") + " input[type=checkbox]",
      ),
  ...
  function setPosition(lon, lat) {
    ...
    var leftHandBounds = [
      new OpenLayers.Bounds(-9, 50, 3, 60), // British Isles
  ...
  function setPositionByString() {
    ...
    $.ajax({
      url:
        ...
        query +
        "&format=json&polygon=0&addressdetails=0&limit=1&callback",
      cache: false,
      dataType: "json",
  ...
  function connectSocket() {
    ...
    socket.onerror = function (error) {
      if (presentedErrorLog == false) {
        window.alert(
          "Socket connection failed. Please open the OSM WebWizard by using osmWebWizard.py or the link in your start menu.",
        );
    ...
    socket.addEventListener("message", function (evt) {
      ...
      if (type === "zip") {
        ...
        elem("#status > div").style.width =
          (100 * currentStep) / totalSteps + "%";
  ...
  function startBuild() {
    ...
    } else data.coords = [cor.left, cor.bottom, cor.right, cor.top];

    vehicleClasses.forEach(function (vehicleClass) {
      var result = vehicleClass.toJSON();
      if (result) data.vehicles[vehicleClass.internal] = result;
    });
    ...
    roadClasses.forEach(function (roadType) {
      ...
      if (result) {
        ...
        if (
          roadType.category == "Pedestrians" ||
          roadType.category == "Highway"
        ) {
          try {
            data.roadTypes["Highway"] =
              data.roadTypes["Highway"].concat(result);
          } catch (e) {
            if (e instanceof TypeError) {
              data.roadTypes["Highway"] = result;
            }
          }
        } else data.roadTypes[roadType.category] = result;
      }
    });
